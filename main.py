...
class GUI(QtWidgets.QMainWindow):
    ...
    def add_new_node(self):
        ...
        if not new_node:
            ...
            self.show_dialog("Casilla vacia.")
            return
        
        self.form.newnode_text.clear()
        
        if self.G.has_node(new_node):
            self.show_dialog(f"{new_node} Ya está construido.")
    ...
    def connect_nodes(self):
        ...
        try:
            ...
            self.show_dialog("El peso debe ser un entero.")
            return

        if self.G.has_edge(node1, node2):
            self.show_dialog(f"Unión: {node1, node2} Ya están construidos.")  
    ...
    def run(self):
        ...
        if self.G.has_node(source) and self.G.has_node(dest):
            if source in nx.algorithms.descendants(self.G, dest):
                ...
                result = f"El resultado total = {visited[dest]}.\nEl camino más corto entre {source} y {dest} es: {shortest_path}"
